<template>
  <Row type="flex">
    ...
    <Panel id="contest-card" shadow>
      <div slot="title">{{query.rule_type === '' ? 'Все' : query.rule_type}} Соревнования</div>
      <div slot="extra">
        <ul class="filter">
          <!--<li>-->
            <!--<Dropdown @on-click="onRuleChange">-->
              <!--<span>{{query.rule_type === '' ? 'Режим' : query.rule_type}}-->
          ...
          <li>
            <Dropdown @on-click="onStatusChange">
              <span>{{query.status === '' ? 'Состояние' : CONTEST_STATUS_REVERSE[query.status].name}}
                <Icon type="arrow-down-b"></Icon>
              </span>
              <Dropdown-menu slot="list">
                <Dropdown-item name="">Все</Dropdown-item>
      ...
      <p id="no-contest" v-if="contests.length == 0">Соревнований нет</p>
      <ol id="contest-list">
